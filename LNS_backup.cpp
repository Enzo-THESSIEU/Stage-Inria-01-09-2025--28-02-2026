...
//   int instance_id = atoi(argv[1]);
    ...
    int total_subsets = int((size_of_set - size_of_subset) / overlap_size) + 1; 
	...
	std::string current_file = print_n_tw_score_requests(instance_id, 0, size_of_subset, size_of_set);

	std::cout << "Subset " << 1 << " information gathered \n";

	struct problem p_current;
	build_first_problem(p_current, current_file);
	std::cout << "Problem Built for Subset " << 1 << " out of "<< total_subsets + 1 << "\n";
    ...
    for (int subset_number = 1 ; subset_number < total_subsets - 1 ; subset_number++){
		...
		current_file = print_n_tw_score_requests(instance_id, subset_number, size_of_subset, size_of_set);
		...
		auto [phantom_pickup_nodes, phantom_dropoff_nodes] = remove_nodes_route(s_previous, p_previous, previous_file, current_file);

		build_sequential_problem(p_current, current_file, p_previous, s_previous, phantom_pickup_nodes, phantom_dropoff_nodes);
...
std::vector<int> get_instersecting_requests(std::string previous_file, std::string current_file){
    ...
    // vector<int> current_pickup_nodes;
	// vector<int> current_dropoff_nodes;
	// vector<int> previous_pickup_nodes;
	// vector<int> previous_dropoff_nodes;
	vector<int> current_actual_requests;
    ...
    while (infile_previous1 >> node >> x_coord >> y_coord >> lower_tw >> upper_tw >> mobile >> wheelchair >> service_dur >> actual_node) {
        if (actual_node < n_requests){
            previous_actual_requests.push_back(actual_node);
        }
    }
    ...
    while (infile_current1 >> node >> x_coord >> y_coord >> lower_tw >> upper_tw >> mobile >> wheelchair >> service_dur >> actual_node) {
        if (actual_node < n_requests){
            current_actual_requests.push_back(actual_node);
        }
    }

	vector<int> intersecting_requests;

	for (int current_request : current_actual_requests){
		if (std::find(previous_actual_requests.begin(), previous_actual_requests.end(), current_request) != previous_actual_requests.end()) {
			intersecting_requests.push_back(current_request);
		}
	}

	// std::ifstream infile_previous2(previous_file);
	// if (!infile_previous2.is_open()) {
	// 	throw std::runtime_error("File not opened: " + previous_file);
	// }
    // while (infile_previous2 >> node >> x_coord >> y_coord >> lower_tw >> upper_tw >> mobile >> wheelchair >> service_dur >> actual_node) {
    //     if (std::find(intersecting_requests.begin(), intersecting_requests.end(), actual_node) != intersecting_requests.end()){
	// 		if (node < n_requests){
    //         	previous_pickup_nodes.push_back(node);
    //     	}
	// 		if (node >= n_requests && node < 2 * n_requests){
    //         	previous_dropoff_nodes.push_back(node);
    //     	}
    // 	}
	// }

	// std::ifstream infile_current2(current_file);
	// if (!infile_current2.is_open()) {
	// 	throw std::runtime_error("File not opened: " + current_file);
	// }
    // while (infile_current2 >> node >> x_coord >> y_coord >> lower_tw >> upper_tw >> mobile >> wheelchair >> service_dur >> actual_node) {
    //     if (std::find(intersecting_requests.begin(), intersecting_requests.end(), actual_node) != intersecting_requests.end()){
	// 		if (node < n_requests){
    //         	current_pickup_nodes.push_back(node);
        	
	// 		if (node >= n_requests && node < 2 * n_requests){

    //         	current_dropoff_nodes.push_back(node);
    //     	}
    // 	}
	...
	// std::vector<std::vector<int>> nodes = {current_pickup_nodes, current_dropoff_nodes, previous_pickup_nodes, previous_dropoff_nodes};
...
static inline void unlink_ref(solution &s, int ref) {
    ...
    if (prev != -1) s.successor[prev] = next;
    if (next != -1) s.predecessor[next] = prev;

    s.predecessor[ref] = -1;
    s.successor[ref]   = -1;
    s.map_to_vehicle[ref] = -1; // now "not in any vehicle route"
}
...
pair<vector<pair<int, int>> , vector<pair<int, int>>> remove_nodes_route(solution &s, problem &p, std::string previous_file, std::string current_file){
	vector<int> intersecting_requests = get_instersecting_requests(previous_file, current_file);
	...
	for (int req : intersecting_requests) {
		...
		// membership
        const bool req_in_P  = (find_node_by_req(removable_pickups, request_id) != nullptr);
        const bool req_in_D  = (find_node_by_req(removable_dropoffs, request_id) != nullptr);
        const bool req_in_T1 = (find_node_by_req(removable_firstleg_transfers, request_id) != nullptr);
        const bool req_in_T2 = (find_node_by_req(removable_secondleg_transfers, request_id) != nullptr);
		...
		if (req_in_P && req_in_D){
			...
			// Case 1b: P and D aren't on the same vehicle route (No Phantom node needed)
			if (node_p->vehicle_number != node_d->vehicle_number){
...
std::vector<int> get_final_vehicle_positions(solution s, problem &p){
    ...
    for (int k = 0; k < p.n_vehicles; k++){
        ...
        final_positions[k] = s.predecessor[ref];
    }
...
void build_sequential_problem(struct problem &current_p, std::string data_darp, problem previous_p, solution &previous_s, vector<pair<int, int>> phantom_pickup_nodes, vector<pair<int, int>> phantom_dropoff_nodes) {
...
void read_sequential_data_darp(problem &current_p, const std::string& data_darp, solution previous_s, problem previous_p) {
	...
	vector<int> previous_final_positions;
	...
	while (infile >> node >> x_coord >> y_coord >> lower_tw >> upper_tw >> mobile >> wheelchair >> service_dur >> actual_node) {
		...
		// Data depot nodes and vehicles:
			...
			// std::cout << "For vehicle " << vehicle << " the previous final position is " << previous_final_positions[vehicle] << "\n";

			// Copy depot coordinates/TW from the previous solution final position of this vehicle
            struct node prev_depot_like = previous_p.nodes[ previous_final_positions[vehicle] ];

            current_p.nodes[node] = {
                prev_depot_like.x_coord,
                prev_depot_like.y_coord,
                prev_depot_like.lower_tw,
                prev_depot_like.upper_tw,
...
// void initialize_solution_no_depot(struct problem &p, struct solution &s) {
...
// void write_subproblem_output_file(struct problem &p, struct solution &s, int current_iteration, double computation_time, bool extensive_output, string file){
...
// 	output_file << "=============================================\n"
// 				<< "=============================================\n"
// 				<< "The Subproblem for the following subset is : \n"
// 				<< "=============================================\n";

// 	for (node inf_node : p.nodes)
// 	output_file <<
								 ...
								 int subproblem_number)
{
    ...
    output_file.open("updated_" + file, std::ios_base::app);
...
{
    std::ofstream output_file;
    output_file.open("results_" + file, std::ios_base::app);
    if (!output_file.is_open()) return;
