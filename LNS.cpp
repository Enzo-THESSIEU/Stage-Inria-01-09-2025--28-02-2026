...
// Input data:
// vector<string> data_darp = {"datafile0.txt"};
// int instance_id = 1;
...
//   int instance_id = atoi(argv[1]);
    ...
    instance_info info_of_instance = get_instance_info_pipeline(instance_id);

	std::cout << "get_instance_info_pipeline Completed \n";

    int size_of_subset = 20;
	int overlap_size = 10;
    int size_of_set = info_of_instance.n_requests;
	int offset = 0;

	std::cout << "Set size : " << size_of_set << "\n";
    
    int total_subsets = int((size_of_set - size_of_subset) / overlap_size) ; 

	std::string previous_file;
	struct problem p_previous;
	struct solution s_previous;

	std::vector<std::pair<problem, solution>> all_subset_info;

	// Run first subset

	std::string current_file = print_n_tw_score_requests(instance_id, 0, size_of_subset, size_of_set, offset);

	std::cout << "Subset " << 1 << " information gathered \n";

	struct problem p_current;
	build_first_problem(p_current, current_file);
	std::cout << "Problem Built for Subset " << 1 << " out of "<< total_subsets << "\n";

	solution s_current = run_for_subproblem(argc, argv, current_file, p_current, 1);
	std::cout << "Problem ran for Subset " << 1 << " out of "<< total_subsets + 1 << "\n";

	tot_dist += s_current.total_distance;

	all_subset_info.push_back({p_current, s_current});

	// total_subsets = 2;

    for (int subset_number = 1 ; subset_number < total_subsets + 1; subset_number++){
		bool is_final_subset = (subset_number == total_subsets);
		previous_file = current_file;
		offset = subset_number * overlap_size;
		current_file = print_n_tw_score_requests(instance_id, subset_number, size_of_subset, size_of_set, offset);

		std::cout << "Subset " << subset_number + 1 << " information gathered \n";

		// Create and initialize problem:
		p_previous = p_current;
		s_previous = s_current;

		write_solution_to_file(s_current, p_current.n_nodes, subset_number, 0);
		std::cout<<"\n";
		std::cout<<"\n";
		auto [phantom_pickup_nodes, phantom_dropoff_nodes] = remove_nodes_route(s_previous, p_previous, previous_file, current_file, overlap_size);
		tot_dist += s_previous.total_distance;  // Total distance calculated after node removal
		std::cout<<"\n";
		std::cout<<"\n";
		write_solution_to_file(s_current, p_current.n_nodes, subset_number, 1);

		build_sequential_problem(p_current, current_file, p_previous, s_previous, phantom_pickup_nodes, phantom_dropoff_nodes, is_final_subset);
		std::cout << "Problem Built for Subset " << subset_number + 1 << " out of "<< total_subsets + 1 << "\n";

		s_current = run_for_subproblem(argc, argv, current_file, p_current, subset_number);
		std::cout << "Problem Ran for Subset " << subset_number + 1 << " out of "<< total_subsets + 1 << "\n";
		std::cout << "\n";
		std::cout << "\n";
	...
	chrono::duration<double> diff = total_end_time - total_start_time;
	double total_time = diff.count();
	std::cout << "Total Distance is: " << tot_dist << "\n";
	std::cout << "Total Time is: " << total_time << "\n";


	write_full_solution_results_file(all_subset_info);
...
{
    OUTPUT_DIR = fs::path("outputs");
    fs::create_directories(OUTPUT_DIR);
}


// std::vector<int> get_instersecting_requests(std::string previous_file, std::string current_file){

//     int n_requests = 200;
// 	int a1, a2, a3;
//     int node, mobile, wheelchair, actual_node;
// 	double x_coord, y_coord, lower_tw, upper_tw, service_dur;

// 	vector<pair<int, int>> current_actual_requests;
// 	vector<pair<int, int>> previous_actual_requests;

// 	std::cout << "==============\n";
// 	std::cout << "Previous File \n";
// 	std::cout << "==============\n";

// 	std::ifstream infile_previous1(previous_file);
// 	if (!infile_previous1.is_open()) {
// 		throw std::runtime_error("File not opened: " + previous_file);
// 	}
// 	infile_previous1 >> a1 >> a2 >> a3;

//     while (infile_previous1 >> node >> x_coord >> y_coord >> lower_tw >> upper_tw >> mobile >> wheelchair >> service_dur >> actual_node) {
// 		std::cout << "Subproblem Node Id: " << node << " , Actual Node Id: " << actual_node << "\n";
//         if (actual_node < n_requests){
//             previous_actual_requests.push_back({node, actual_node});
//         }
//     }
...
// 	std::ifstream infile_current1(current_file);
// 	if (!infile_current1.is_open()) {
// 		throw std::runtime_error("File not opened: " + current_file);
// 	}
// 	infile_current1 >> a1 >> a2 >> a3;

//     while (infile_current1 >> node >> x_coord >> y_coord >> lower_tw >> upper_tw >> mobile >> wheelchair >> service_dur >> actual_node) {
// 		std::cout << "Subproblem Node Id: " << node << " , Actual Node Id: " << actual_node << "\n";
//         if (actual_node < n_requests){
//             current_actual_requests.push_back({node, actual_node});
//         }
//     }

// 	std::cout << "Previous Requests \n";
// 	for (auto previous_request : previous_actual_requests){
// 		std::cout << "Subproblem Node Id: " << previous_request.first << " , Actual Node Id: " << previous_request.second << "\n";
// 	}
// 	std::cout << "Current Requests \n";
// 	for (auto previous_request : current_actual_requests){
// 		std::cout << "Subproblem Node Id: " << previous_request.first << " , Actual Node Id: " << previous_request.second << "\n";
// 	}

// 	vector<int> intersecting_requests;

// 	for (auto previous_request : previous_actual_requests){
// 		int target = previous_request.second;

// 		auto it = std::find_if(current_actual_requests.begin(), current_actual_requests.end(),
// 							[&](const std::pair<int,int>& x){
// 								return x.second == target;
// 							});

// 		if (it != current_actual_requests.end()) {
// 			// found
// 		}

// 	}

//     return intersecting_requests;
// }


std::vector<int> get_intersecting_requests(std::string previous_file, int overlap_size){

	int n_vehicles, n_requests, n_terminals;
    int node, mobile, wheelchair, actual_node;
	double x_coord, y_coord, lower_tw, upper_tw, service_dur;

	vector<int> previous_actual_requests;

	std::ifstream infile_previous1(previous_file);
	if (!infile_previous1.is_open()) {
		throw std::runtime_error("File not opened: " + previous_file);
	}
	infile_previous1 >> n_vehicles >> n_requests >> n_terminals;

    while (infile_previous1 >> node >> x_coord >> y_coord >> lower_tw >> upper_tw >> mobile >> wheelchair >> service_dur >> actual_node) {
		// std::cout << "Subproblem Node Id: " << node << " , Actual Node Id: " << actual_node << "\n";
        if (node < n_requests){
            previous_actual_requests.push_back(node);
        }
    }

	int num_nodes = previous_actual_requests.size();

	int start = num_nodes - overlap_size;
	int end   = num_nodes + 1; // +1 because end iterator is exclusive

	std::vector<int> intersecting_requests(
		previous_actual_requests.begin() + start,
		previous_actual_requests.begin() + end
	);
...
// ======================= Vehicle Routes =========================
// Returns (node_id, ref) along the route
std::vector<std::pair<int, int>> get_vehicle_routes(struct problem &p, struct solution &s, int vehicle_number){
    std::vector<std::pair<int, int>> vehicle_route = {};
    int begin_depot_ref = vehicle_number + 2 * p.n_requests;
    int ref = begin_depot_ref;
    int end_depot_ref = begin_depot_ref + p.n_vehicles;

    // push start depot
    vehicle_route.push_back({s.map_to_node[ref], ref});

    while (ref != end_depot_ref){
        ref = s.successor[ref];                         // move to next ref
        vehicle_route.push_back({s.map_to_node[ref], ref}); // store (node, ref)
    }
...
std::pair<std::vector<std::pair<int,int>>, std::vector<std::pair<int,int>>> break_up_vehicle_route(solution s, problem &p, std::vector<std::pair<int,int>> vehicle_route){
    // Find the position of the last node in vehicle_route (vector<node_id, reference>) where
    // ref ∈ [0, n_requests/2] ∪ [n_requests, n_requests + n_requests/2] ∪ [2*n_requests, 2*n_requests + n_vehicles/2]
    // Break the route into two parts named constant_route and removable_route

    int last_pos = -1;

    for (int i = static_cast<int>(vehicle_route.size()) - 1; i >= 0; --i) {
        int ref = vehicle_route[i].second;

        bool in_first_pickups  = (ref >= 0 && ref < p.n_requests / 2);
        bool in_first_dropoffs = (ref >= p.n_requests && ref < p.n_requests + p.n_requests / 2);
        bool in_first_depots   = (ref >= 2 * p.n_requests && ref < 2 * p.n_requests + p.n_vehicles / 2);

        if (in_first_pickups || in_first_dropoffs || in_first_depots) {
            last_pos = i;
            break;
        }
    }

    std::vector<std::pair<int,int>> constant_route;
    std::vector<std::pair<int,int>> removable_route;

    if (last_pos == -1) {
        // nothing matched -> decide your behavior
        constant_route = vehicle_route;
        removable_route = {};
    } else {
        // Up to you if last_pos goes in constant or removable; here it's kept in constant
        constant_route.assign(vehicle_route.begin(), vehicle_route.begin() + last_pos + 1);
        removable_route.assign(vehicle_route.begin() + last_pos + 1, vehicle_route.end());
    }

    // TODO: return what you want (both routes? only removable? pack them?)
    // Common quick hack if you must return one vector: return removable_route;
    return {constant_route, removable_route};
}



// --------- low-level route editing on successor/predecessor ---------
static inline void unlink_ref(solution &s, int ref) {
    const int prev = s.predecessor[ref];
    const int next = s.successor[ref];

	s.successor[prev] = s.successor[ref];
	s.predecessor[next] = s.predecessor[ref];

	s.successor[ref] = -1;
	s.predecessor[ref] = -1;
	s.map_to_vehicle[ref] = -1;
	s.service_start[ref] = -1;

}



static inline removable_node_info *find_node_by_req(vector<removable_node_info> &v, int request_id) {
    auto it = std::find_if(v.begin(), v.end(),
                           [&](const removable_node_info &n){ return n.request_id == request_id; });
    return (it == v.end()) ? nullptr : &(*it);
}



pair<vector<pair<int, int>> , vector<pair<int, int>>> remove_nodes_route(solution &s, problem &p, std::string previous_file, std::string current_file, int overlap_size){
	vector<int> intersecting_requests = get_intersecting_requests(previous_file, overlap_size);

	// std::cout << "Intersecting Requests are : [ ";
	// for (int req : intersecting_requests){
	// 	std::cout << req << ", ";
	// }
	// std::cout << " ] \n";

	removable_node_info node_inf;
	vector<removable_node_info> removable_dropoffs  = {};
	vector<removable_node_info> removable_pickups   = {};
	vector<removable_node_info> removable_firstleg_transfers = {};
	vector<removable_node_info> removable_secondleg_transfers = {};

	int ref;

	for (int vehicle_num = 0 ; vehicle_num < p.n_vehicles ; vehicle_num++){
		vector<pair<int,int>> vehicle_route = get_vehicle_routes(p, s, vehicle_num);
		auto [constant_route, removable_route] = break_up_vehicle_route(s, p, vehicle_route);
		for (std::pair<int, int> node : removable_route){
			ref = node.second;

			bool in_first_pickups  = (ref >= 0 && ref < p.n_requests / 2);
			bool in_last_pickups =  (ref >= p.n_requests / 2 && ref < p.n_requests);
			bool in_first_dropoffs = (ref >= p.n_requests && ref < p.n_requests + p.n_requests / 2);
			bool in_last_dropoffs = (ref >= p.n_requests + p.n_requests / 2 && ref < 2 * p.n_requests);
			bool in_start_depots   = (ref >= 2 * p.n_requests && ref < 2 * p.n_requests + p.n_vehicles);
			bool in_end_depots   = (ref >= 2 * p.n_requests + p.n_vehicles && ref < 2 * p.n_requests + 2 * p.n_vehicles);
			bool in_firstleg_transfers    = (ref >= 2 * p.n_requests + 2* p.n_vehicles && ref < 2 * p.n_requests + 2 * p.n_vehicles + p.n_terminals);
			bool in_secondleg_transfers    = (ref >= 2 * p.n_requests + 2* p.n_vehicles + p.n_terminals && ref < 2 * p.n_requests + 2 * p.n_vehicles + 2 * p.n_terminals);

			node_inf.reference = node.second;
			node_inf.node_id_problem = node.first;
			node_inf.vehicle_number = vehicle_num;
			node_inf.request_id = s.map_to_request[ref];
			if (in_last_dropoffs) {
				node_inf.node_type = "dropoff";
				removable_dropoffs.push_back(node_inf);
			}
			else if (in_last_pickups){
				node_inf.node_type = "pickup";
				removable_pickups.push_back(node_inf);
			}
			else if (in_firstleg_transfers){
				node_inf.node_type = "transfer";
				removable_firstleg_transfers.push_back(node_inf);
			}
			else if (in_secondleg_transfers){
				node_inf.node_type = "transfer";
				removable_secondleg_transfers.push_back(node_inf);
			}
	...
	// std::cout << "=== Removable Pickup Nodes === \n";
	// for (removable_node_info node : removable_pickups){
	// 	std::cout << "Following id is removable: " << node.node_id_problem << "\n";
	// }

	// std::cout << "=== Removable Dropoff Nodes === \n";
	// for (removable_node_info node : removable_dropoffs){
	// 	std::cout << "Following id is removable: " << node.node_id_problem << "\n";
	// }

	// std::cout << "=== Removable First Leg Transfer Nodes === \n";
	// for (removable_node_info node : removable_firstleg_transfers){
	// 	std::cout << "Following id is removable: " << node.node_id_problem << "\n";
	// }

	// std::cout << "=== Removable Second Leg Pickup Nodes === \n";
	// for (removable_node_info node : removable_secondleg_transfers){
	// 	std::cout << "Following id is removable: " << node.node_id_problem << "\n";
	// }


	vector<pair<int, int>> phantom_pickup_nodes = {};
	vector<pair<int, int>> phantom_dropoff_nodes = {};
	...
	for (int req : intersecting_requests) {
		...
		auto it_p  = std::find_if(removable_pickups.begin(), removable_pickups.end(),
								[&](const removable_node_info &n){ return n.request_id == request_id; });
		auto it_d  = std::find_if(removable_dropoffs.begin(), removable_dropoffs.end(),
								[&](const removable_node_info &n){ return n.request_id == request_id; });
		auto it_t1 = std::find_if(removable_firstleg_transfers.begin(), removable_firstleg_transfers.end(),
								[&](const removable_node_info &n){ return n.request_id == request_id; });
		auto it_t2 = std::find_if(removable_secondleg_transfers.begin(), removable_secondleg_transfers.end(),
								[&](const removable_node_info &n){ return n.request_id == request_id; });

		bool req_in_P  = (it_p  != removable_pickups.end());
		bool req_in_D  = (it_d  != removable_dropoffs.end());
		bool req_in_T1 = (it_t1 != removable_firstleg_transfers.end());
		bool req_in_T2 = (it_t2 != removable_secondleg_transfers.end());


		// std::cout << "Request id: " << request_id << " , req_in_P: " << req_in_P << " , req_in_D: " << req_in_D << " , req_in_t1: " << req_in_T1 << " , req_in_t2: " << req_in_T2 << "\n";

		if (req_in_P && req_in_D){
			// Case 1: Both P and D are removable
			auto node_p = find_node_by_req(removable_pickups, request_id);
			auto node_d = find_node_by_req(removable_dropoffs, request_id);

			// Case 1a: P and D are on the same vehicle route (No Phantom node needed)
			if (node_p->vehicle_number == node_d->vehicle_number){
				// std::cout << "Case 1a for request: " << request_id << " (" << p.nodes[node_p->node_id_problem].actual_node << ", " << p.nodes[node_d->node_id_problem].actual_node << ")\n";
				unlink_ref(s, node_p->reference);
				unlink_ref(s, node_d->reference);
			}

			// Case 1b: P and D aren't on the same vehicle route and T2 is removable (if P removable so is T1) (No Phantom node needed)
			if (node_p->vehicle_number != node_d->vehicle_number){
				// Case 1b: P and D aren't on the same vehicle route and T2 is removable (if P removable so is T1) (No Phantom node needed)
				if (req_in_T2){
					auto node_t1 = find_node_by_req(removable_firstleg_transfers, request_id);
					auto node_t2 = find_node_by_req(removable_secondleg_transfers, request_id);

					// std::cout << "Case 1b : (" << p.nodes[node_p->node_id_problem].actual_node << ", " << p.nodes[node_d->node_id_problem].actual_node <<
					// ") and (" << p.nodes[node_t1->node_id_problem].actual_node << ", " << p.nodes[node_t2->node_id_problem].actual_node << ")\n";

					unlink_ref(s, node_p->reference);
					unlink_ref(s, node_d->reference);
					unlink_ref(s, node_t1->reference);
					unlink_ref(s, node_t2->reference);
				}

				// Case 1c: P and D aren't on the same vehicle route and T2 is not removable 
				else if (!req_in_T2){// Look later at this case
				}
		...
		// Case 2: Only the dropoff node is removable
		if (!req_in_P && req_in_D){
			auto node_d = find_node_by_req(removable_dropoffs, request_id);

			int corresponding_pickup_node_ref = s.map_to_request[node_d->reference];
			int corresponding_p_vehicle_number = s.map_to_vehicle[corresponding_pickup_node_ref];

			// Case 2a: P and D are on the same route, a phantom node for p is needed and is placed at the end of the route where the pickup is done
			if (corresponding_p_vehicle_number == node_d->vehicle_number){
				// std::cout << "Case 2a : (" << p.nodes[node_d->node_id_problem].actual_node << ")\n";
				unlink_ref(s, node_d->reference);
				phantom_pickup_nodes.push_back({node_d->vehicle_number, node_d->node_id_problem});
			}
			...
			if (corresponding_p_vehicle_number != node_d->vehicle_number){
				...
				// Case 2b: D, T1, T2 are removable, a phantom node is placed at the end of the route where the pickup is done
				if (req_in_T1 && req_in_T2){
					// std::cout << "Case 2b : (" << p.nodes[node_d->node_id_problem].actual_node <<
					// ") and (" << p.nodes[node_t1->node_id_problem].actual_node << ", " << p.nodes[node_t2->node_id_problem].actual_node << ")\n";

					unlink_ref(s, node_d->reference);
					unlink_ref(s, node_t1->reference);
					unlink_ref(s, node_t2->reference);
					phantom_pickup_nodes.push_back({node_t1->vehicle_number, node_t1->node_id_problem});

				}

				// Case 2c: D is removable and T1 isn't removable (T2 has no importance), a phantom node p is placed at the end of route of the vehicle passing through t2 (t2 is not removed)
				if (!req_in_T1){
					// std::cout << "Case 2c : (" << p.nodes[node_d->node_id_problem].actual_node << ")\n";
					unlink_ref(s, node_d->reference);
					phantom_pickup_nodes.push_back({node_t2->vehicle_number, node_t2->node_id_problem});

				}

			}
		// Case 3: Only the pickup node is removable
		if (req_in_P && !req_in_D){
			continue;
			// auto node_p = std::find_if(removable_pickups.begin(), removable_pickups.end(),
			// 						[&](const removable_node_info &n){
			// 							return n.request_id == request_id;
			// 						});

			// auto node_t1 = std::find_if(removable_firstleg_transfers.begin(),
			// 			removable_firstleg_transfers.end(),
			// 			[&](const removable_node_info &n){
			// 				return n.request_id == request_id;
			// 			});


			// auto node_t2 = std::find_if(removable_secondleg_transfers.begin(),
			// 			removable_secondleg_transfers.end(),
			// 			[&](const removable_node_info &n){
			// 				return n.request_id == request_id;
			// 			});


			// int corresponding_dropoff_node_ref = node_p.reference +  p.n_requests;
			// int corresponding_p_vehicle_number = s.map_to_vehicle[corresponding_dropoff_node_ref];

			// unlink_ref(s, node_p.reference);

			// phantom_dropoff_nodes.push_back(node_t1.vehicle_number, node_t1.node_id_problem)
	...
	return {phantom_pickup_nodes, phantom_dropoff_nodes};


	


	// Case 1: Pickup and dropoff in same route with format P -->  ... --> D

	// Case 1a: Both P and D in removable node sets
	// Both P and D can be removed

	// Case 1b: Only D in removable node sets
	// Only D can be removed, P needs to be added to phantom nodes set with mention of which vehicle passed through node P in order to mantain passenger continuity


	// Case 2: Pickup and and dropoff not in same route with format  P -->  ... --> T1 --> T2 -->  ... --> D

	// Case 2a: Only D in removable node sets
	// Only D can be removed, D is removed and a P is added to phantom nodes set with mention of the vehicle from which D was removed

	// Case 2b: T2 and D in removable node sets
	// D and T2 can be removed but not Ti, only D is removed with a phantom node P being added to the phantom node set with mention of the vehicle D was removed from.

	// Case 2c: D, T2 and T1 in removable node sets
	// D, T2 and T1 can be removed, so all three nodes are removed and a phantom node P is added to the phantom node set with the mention of the vehicle the node P was visited

	// Case 2d: All nodes in removable node sets
	// All nodes can be removed so the whole request is removed

	// Case 2e: P and T1 can be removed but not T2 and D. P and T1 are removed but a phantom node D is added at Ti with a time window constraint reinforcement  ( Ignore this case for the time being)
}


std::vector<std::pair<int, double>> get_final_vehicle_positions(solution s, problem &p){
    std::vector<std::pair<int, double>> final_positions;
    final_positions.reserve(p.n_vehicles);
    for (int k = 0; k < p.n_vehicles; k++){
        int ref = 2 * p.n_requests + p.n_vehicles + k;
        final_positions[k] = {s.predecessor[ref], s.service_start[s.predecessor[ref]]};
    }
...
std::vector<std::vector<std::pair<int, double>>> reconstruct_full_routes(const std::vector<std::pair<problem, solution>> &all_subset_info){
	int n_vehicles = all_subset_info.at(0).first.n_vehicles;

	std::vector<std::vector<pair<int, double>>> all_routes;
	all_routes.reserve(n_vehicles);

	for (int vehicle = 0 ; vehicle < n_vehicles ; vehicle++){
		std::vector<pair<int, double>> vehicle_route;

		for (const auto &subset_info : all_subset_info) {
					const problem  &p = subset_info.first;
					const solution &s = subset_info.second;

					std::vector<std::pair<int, int>> partial_route = get_vehicle_routes(const_cast<problem&>(p), const_cast<solution&>(s), vehicle);

			for (auto [node_id, ref] : partial_route){
				int real_node_id = p.nodes[node_id].actual_node;
				

				if (real_node_id >= 0 && real_node_id < 2 * p.n_requests){
					auto it = std::find_if(
                        vehicle_route.begin(), vehicle_route.end(),
                        [&](const std::pair<int,double> &info_of_node) {
                            return info_of_node.first == real_node_id;
                        });

					if (it == vehicle_route.end()){
						vehicle_route.push_back({real_node_id, s.service_start[ref]});
					}
				}
				else{vehicle_route.push_back({real_node_id, s.service_start[ref]});}

			}
		}
		all_routes.push_back(vehicle_route);
	}
	return all_routes;
}
...
// Main function to run the algorithm for a subproblem:
solution run_for_subproblem(int argc, char *argv[], string file, problem p, int subproblem_number) {
	...
	write_subproblem_output_file(p, file, subproblem_number);
	...
	std::cout << "Modifying service start time for all start Depot nodes: ";

	initialize_solution(p, s_curr);

	// std::cout << "Service start times for Depot nodes are: \n";
	// for (int vehicle = 0; vehicle < p.n_vehicles; vehicle++ ){
	// 	std::cout << "Service start time for vehicle after intialise_solution " << vehicle << " is ";
	// 	int vehicle_ref = vehicle + 2 * p.n_requests;
	// 	std::cout << s_curr.service_start[vehicle_ref] << "\n";
	// }
	// std::cout << "\n";
	...
	// 	std::cout << "Service start time for vehicle after intialise_solution " << vehicle << " is ";
	// 	int vehicle_ref = vehicle + 2 * p.n_requests;
	// 	std::cout << s_best.service_start[vehicle_ref] << "\n";
	// }
	// std::cout << "\n";
	...
	// 	std::cout << "Service start time for vehicle after intialise_solution " << vehicle << " is ";
	// 	int vehicle_ref = vehicle + 2 * p.n_requests;
	// 	std::cout << s_over.service_start[vehicle_ref] << "\n";
	// }
	...
	// 	std::cout << "Service start time for vehicle after intialise_solution " << vehicle << " is ";
	// 	int vehicle_ref = vehicle + 2 * p.n_requests;
	// 	std::cout << s_curr.service_start[vehicle] << "\n";
	// }
	...
	for (int iteration = 0; iteration < iterations; iteration++) {
		...
		// 	solution s_diversification_best = s_best;
		// 	solution s_diversification_curr = s_curr;

		// 	do {
		// 		random_removal(p, s_diversification_best, max_removal_pct, 0);
		// 		random_order_insertion(p, s_diversification_best);
		// 	} while (check_completeness(p, s_diversification_best) == false);
		// 	relocate(p, s_diversification_best);
		// 	exchange_natural_sequences(p, s_diversification_best);

		// 	iter_last_impr = iteration;
		// 	update_solution(p, s_diversification_curr, s_diversification_best, true);
		// 	printf("Diversification to %.2f in iteration %d \n", s_diversification_best.total_distance, iteration + 1);
		// }
		...
		if (check_completeness(p, s_curr) == false) { update_solution(p, s_curr, s_best, true); }
			...
			if (check_completeness(p, s_curr) == false ) { update_solution(p, s_curr, s_best, true); 
				// printf("ERROR - Incomplete solution corrected\n"); 
			}
			...
			// Update the best solution if needed:
				...
				if (check_feasibility(p, s_best) == false) { 
					// printf("ERROR - Infeasible best solution accepted\n"); 
				}
	...
	write_subproblem_solution_output_file(p, s_over, file, subproblem_number);
...
void build_first_problem(struct problem &p, std::string data_darp) {
	...
	std::cout << "Data Read for " << data_darp << "\n";
	
	// Preprocessing steps:
	compute_matrix_nontightened(p);
	
	std::cout << "Matrix Computed for " << data_darp << "\n";
	...
	std::cout << "Matrix Tightened for " << data_darp << "\n";
}
...
void read_data_darp(problem &p, const std::string& data_darp) {
	...
	std::cout << "p.n_requests " << p.n_requests << "\n";
	std::cout << "p.n_vehicles " << p.n_vehicles << "\n";
	std::cout << "p.n_terminals " << p.n_terminals << "\n";
	...
	while (infile >> node >> x_coord >> y_coord >> lower_tw >> upper_tw >> mobile >> wheelchair >> service_dur >> actual_node) {
		
		// Data pickup nodes and requests:
		if (node < p.n_requests) {
			p.nodes[node] = { x_coord, y_coord, lower_tw, upper_tw, service_dur, 0, actual_node };
		...
		// Data delivery nodes:
			...
			p.nodes[node] = { x_coord, y_coord, lower_tw, upper_tw, service_dur, 1, actual_node };
		}

		// Data depot nodes and vehicles:
		else if (node < 2 * p.n_requests + p.n_vehicles) {
			p.nodes[node] = { x_coord, y_coord, lower_tw, upper_tw, service_dur, 2, actual_node }; 
		...
		// Data transfer nodes:
			...
			p.nodes[node] = { x_coord, y_coord, lower_tw, upper_tw, service_dur, 3, actual_node };
		}
	}
...
void build_sequential_problem(struct problem &current_p, std::string data_darp, problem previous_p, solution &previous_s, vector<pair<int, int>> phantom_pickup_nodes, vector<pair<int, int>> phantom_dropoff_nodes, bool is_final_subset) {

	// Read input data:
	read_sequential_data_darp(current_p, data_darp, previous_s, previous_p);

	// std::cout << "Data Read for " << data_darp << "\n";

	compute_matrix_nontightened(current_p);	

	// std::cout << "Matrix Computed for " << data_darp << "\n";

	// Compute the matrix values for all phantom pickup nodes location
	for (pair<int, int> phantom_node : phantom_pickup_nodes){
		auto [vehicle_num, node_id] = phantom_node;
		int entrynode = vehicle_num + 2 * current_p.n_requests;
		compute_phantom_node_as_pickup_node(current_p, node_id, entrynode);
	}

	// std::cout << "Pickup Phantom Nodes Computed for " << data_darp << "\n";

	// Compute the matrix values for all phantom dropoff nodes location
	for (pair<int, int> phantom_node : phantom_dropoff_nodes){
		auto [vehicle_num, node_id] = phantom_node;
		int exitnode = vehicle_num + 2 * current_p.n_requests;
		compute_phantom_node_dropoff_node(current_p, node_id, exitnode);
	}

	// std::cout << "Dropoff Phantom Nodes Computed for " << data_darp << "\n";

	// Compute the matrix values for all the end depot nodes to be of distance 0
	if (! is_final_subset){
		for (int k = 0; k < current_p.n_vehicles; k++){
			compute_phantom_end_depot_node(current_p, k);
		}
	}

	// std::cout << "Depot Phantom Nodes Computed for " << data_darp << "\n";
	
	// Preprocessing steps:
	if (transfers == 1) { compute_matrix_pt(current_p); }
	compute_relative_mrt(current_p);
	if (transfers == 1) { compute_nearest_terminals(current_p); }
	tighten_time_windows(current_p);
	tighten_matrix(current_p);
	compute_user_dissimilarities(current_p);
	compute_node_dissimilarities(current_p);
...
void read_sequential_data_darp(problem &current_p, const std::string& data_darp, solution previous_s, problem previous_p) {
	...
	infile >> current_p.n_vehicles >> current_p.n_requests >> current_p.n_terminals;

	current_p.n_nodes = 2 * current_p.n_requests + current_p.n_vehicles + current_p.n_terminals;
	current_p.n_references = 2 * current_p.n_requests + 2 * current_p.n_vehicles + 2 * current_p.n_requests * transfers;

	current_p.nodes = new node[current_p.n_nodes];
	current_p.requests = new request[current_p.n_requests];
	current_p.vehicles = new vehicle[current_p.n_vehicles];

	int node, mobile, wheelchair, actual_node, type;
	double x_coord, y_coord, lower_tw, upper_tw, service_dur;

	vector<pair<int, double>> previous_final_positions;
	previous_final_positions.reserve(current_p.n_vehicles);
	previous_final_positions = get_final_vehicle_positions(previous_s, previous_p);
	// std::cout << "Previous final positions: " << previous_final_positions << "\n";

	while (infile >> node >> x_coord >> y_coord >> lower_tw >> upper_tw >> mobile >> wheelchair >> service_dur >> actual_node) {
		
		// Data pickup nodes and requests:
		if (node < current_p.n_requests) {
			current_p.nodes[node] = { x_coord, y_coord, lower_tw, upper_tw, service_dur, 0, actual_node };

			int request = node;
			current_p.requests[request] = { mobile, wheelchair, 0.0 };
		}

		// Data delivery nodes:
		else if (node < 2 * current_p.n_requests) {
			current_p.nodes[node] = { x_coord, y_coord, lower_tw, upper_tw, service_dur, 1, actual_node };
		}

		// Data depot nodes and vehicles:
		else if (node < 2 * current_p.n_requests + current_p.n_vehicles) {
			int vehicle = node - 2 * current_p.n_requests;
			
			// std::cout << "For vehicle " << vehicle << " the previous final position is " << previous_final_positions[vehicle].first << "\n";

			// Copy depot coordinates/TW from the previous solution final position of this vehicle
            struct node prev_depot_like = previous_p.nodes[previous_final_positions[vehicle].first];

			double lower_tw;

			if (previous_final_positions[vehicle].second > 718){lower_tw = 0;}  // Temporary manual failsafe for depot --> depot routes (need to find a better criterion)
			else {lower_tw = previous_final_positions[vehicle].second;}

            current_p.nodes[node] = {
                prev_depot_like.x_coord,
                prev_depot_like.y_coord,
                previous_final_positions[vehicle].second,
                720, // prev_depot_like.upper_tw,
                prev_depot_like.service_dur,
                2,                  // depot type
                prev_depot_like.actual_node,
				// prev_depot_like.service_start_time
            };

			current_p.vehicles[vehicle] = { mobile, wheelchair, 720.0 };
		}

		// Data transfer nodes:
		else if (transfers == 1) {
			current_p.nodes[node] = { x_coord, y_coord, lower_tw, upper_tw, service_dur, 3, actual_node };
		}
	}
...
void compute_phantom_end_depot_node(struct problem &p, int phantom_node_id){
	// Computes distance from all nodes to phantom node to be 0, useful for depot nodes so that the route still finishes at that depot node but is virtually in the same place as the penultimate node for that route.
    for (int node1 = 0; node1 < p.n_nodes; node1++) {
        p.matrix_nontightened[node1 * p.n_nodes + phantom_node_id] = 0;
    }
}


void compute_phantom_node_as_pickup_node(struct problem &p, int phantom_node_id, int entrynode){
	// Computes the distance from the entry node to the phantom node as 0 so that it is virtually in the same place
    p.matrix_nontightened[entrynode * p.n_nodes + phantom_node_id] = 0;
	for (int node1 = 0; node1 < p.n_nodes; node1++) {
        p.matrix_nontightened[phantom_node_id * p.n_nodes + node1] = p.matrix_nontightened[entrynode * p.n_nodes + node1];
    }
}
...
void compute_phantom_node_dropoff_node(struct problem &p, int phantom_node_id, int exitnode){
    // Computes the distance from the entry node to the phantom node as 0 so that it is virtually in the same place
    p.matrix_nontightened[phantom_node_id * p.n_nodes + exitnode] = 0;
    // Computes the distance from the phantom node to all other nodes to the same distance as the node from the entry node to all other nodes reinforcing that it is virtually in the same place
    for (int node1 = 0; node1 < p.n_nodes; node1++) {
        p.matrix_nontightened[phantom_node_id * p.n_nodes + node1] = p.matrix_nontightened[exitnode * p.n_nodes + node1];
    }
}


void compute_phantom_start_depot_node(struct problem &p, int phantom_node_id, int depot_node){
	// Computes distance from depot node to be the same as phantom, useful for start depot nodes so that the depot is virtually in the same place as the penultimate node for that route.
    for (int node1 = 0; node1 < p.n_nodes; node1++) {
        p.matrix_nontightened[depot_node * p.n_nodes + node1] = p.matrix_nontightened[phantom_node_id * p.n_nodes + node1];
    }
}
	...
	for (int request1 = 0; request1 < p.n_requests; request1++) {
		for (int request2 = 0; request2 < p.n_requests; request2++) {
			if (request1 != request2) {
				...
				// Some reductions are only valid if no transfers can be performed (Hall et al., 2006):
				if (transfers == 0) {
...
void initialize_solution(struct problem &p, struct solution &s) {
	...
	for (int vehicle = 0; vehicle < p.n_vehicles; vehicle++) {
		...
		// Fix Service start time at Start Depot to be the earliest possible (done for route continuity)
		s.service_start[ref_begin_depot] = p.nodes[ref_begin_depot].lower_tw;
		// std::cout << "For vehicle " << vehicle << " the service start time should be " << p.nodes[ref_begin_depot].lower_tw << "\n";
	}
	...
	for (int vehicle = 0; vehicle < p.n_vehicles; vehicle++) {
		int ref_begin_depot = 2 * p.n_requests + vehicle;
		// std::cout << "For vehicle " << vehicle << " the service start time inside intialise_solution is " << s.service_start[ref_begin_depot] << "\n";
	}
...
void update_solution(struct problem &p, struct solution &s1, struct solution &s2, bool create_schedule) {
	...
	for (int reference = 0; reference < p.n_references; reference++) {
		...
		// Temporary manual failsafe in order to make the service start time for start depot nodes be the earliest possible for route time continuity
	...
	if (create_schedule == true) {
		if (transfers == 1) { 
			check_service_start_BellmanFord(p, s1, true); 

			// Temporary manual failsafe in order to make the service start time for start depot nodes be the earliest possible for route time continuity
			for (int v = 0; v < p.n_vehicles; ++v){
				int begin_depot_ref = v + 2 * p.n_requests;
				s1.service_start[begin_depot_ref] = p.nodes[begin_depot_ref].lower_tw;
			}
		}
...
{
    // 2) build output file path inside that folder
    fs::path out_path = OUTPUT_DIR / ("updated_" + file);

	std::ofstream output_file(out_path, std::ios_base::app);
    if (!output_file.is_open()) return;

    output_file << "=============================================\n"
                << "SUBPROBLEM " << subproblem_number << " OUTPUT\n"
                // << "iteration=" << current_iteration
                // << "  time=" << std::fixed << std::setprecision(3) << computation_time << "s"
                // << "  extensive=" << (extensive_output ? "true" : "false") << "\n"
                << "n_requests=" << p.n_requests
                << "  n_vehicles=" << p.n_vehicles
                << "  n_terminals=" << p.n_terminals
                << "  n_nodes=" << p.n_nodes
                << "  n_references=" << p.n_references << "\n"
                // << "total_distance=" << s.total_distance << "\n"
                << "=============================================\n";

    // --- Nodes ---
    output_file << "\n[NODES]\n";
    output_file << "id\tx\ty\tTWl\tTWu\tserv\ttype\tactual\n";
								 ...
								 int subproblem_number)
{
	...
	// 2) build output file path inside that folder
    fs::path out_path = OUTPUT_DIR / ("results_" + file);
    ...
    // --- Requests ---
    // output_file << "\n[REQUESTS]\n";
    // output_file << "req\tmobile\twheelchair\n";
    // for (int r = 0; r < p.n_requests; ++r) {
    //     const request &rq = p.requests[r];
    //     // Change field names if needed
    //     output_file << r << "\t"
    //                 << rq.mobile << "\t"
    //                 << rq.wheelchair << "\n";
    ...
    // --- Vehicles ---
    // output_file << "\n[VEHICLES]\n";
    // output_file << "veh\tcap_mobile\tcap_wheelchair\tmax_ride\n";
    // for (int v = 0; v < p.n_vehicles; ++v) {
    //     const vehicle &vh = p.vehicles[v];
    //     // Change field names if needed
    //     output_file << v << "\t"
    //                 << vh.mobile << "\t"
    //                 << vh.wheelchair << "\t"
    ...
    // // --- Solution mappings (helpful debugging) ---
    // if (extensive_output) {
    //     output_file << "\n[MAPPINGS by reference]\n";
    //     output_file << "ref\tpred\tsucc\tnode\trequest\tvehicle\tss\tet\tlt\n";
    //     for (int ref = 0; ref < p.n_references; ++ref) {
    //         output_file << ref << "\t"
    //                     << s.predecessor[ref] << "\t"
    //                     << s.successor[ref] << "\t"
    //                     << s.map_to_node[ref] << "\t"
    //                     << s.map_to_request[ref] << "\t"
    //                     << s.map_to_vehicle[ref] << "\t"
    //                     << s.service_start[ref] << "\t"
    //                     << s.earliest_time[ref] << "\t"
    //                     << s.latest_time[ref] << "\n";
    //     }
    ...
    // --- Vehicle routes (most relevant) ---
    output_file << "\n[ROUTES]\n";
    for (int v = 0; v < p.n_vehicles; ++v) {
        auto route = get_vehicle_routes(p, s, v); // vector<pair<node_id, ref>>
        output_file << "vehicle " << v << " (len=" << route.size() << "): ";
        for (const auto &pr : route) {
            output_file << " --> " << pr.first << "(" << s.service_start[pr.second] << ")";
        }
        output_file << "\n";
    }
...
void write_full_solution_results_file(const std::vector<std::pair<problem, solution>> &all_subset_info){
	...
	fs::path out_path = OUTPUT_DIR / ("results");
	...
	int n_vehicles = all_subset_info[0].first.n_vehicles;

	std::vector<std::vector<pair<int, double>>> all_routes = reconstruct_full_routes(all_subset_info);
	int vehicle = 1;
	for (auto route: all_routes){
		output_file << "Vehicle " << vehicle << " route : ";
		for (auto node_ : route){
			output_file << " --> " << node_.first << " (" << node_.second << ") "; 
		}
		output_file << "\n";

		int subproblem = 1;
		for (auto solution_data : all_subset_info){
			std::vector<std::pair<int, int>> partial_route = get_vehicle_routes(solution_data.first, solution_data.second, vehicle - 1);
			output_file << "Route for subproblem " << subproblem;
			for (auto node_ : partial_route){
				output_file << " --> " << solution_data.first.nodes[node_.first].actual_node;
			}
			output_file << "\n";
			subproblem++;
		}
...
{	std::string output_filename = "Solution_" + std::to_string(previous) + "_subset_" + std::to_string(subproblem_number);
	fs::path out_path = OUTPUT_DIR / (output_filename);
