...
import Graph_Builder_2 as g


##### Model Definition #####

def build_idarp_model_Enhanced_2(Time_Limit):
    ...
    for (i,j) in A:
        m.addConstr(
            B_node[j] >= B_node[i] + di[i] + tij[i,j] - M*(1 - v[i,j]),
    ...
    m.update()
    return m, {"x": x, "v": v, "y": y,  "z": z, "B_node": B_node, "B_veh": B_veh, "D_node": D_node}

def build_idarp_model_Enhanced_3(Time_Limit):
...
def build_idarp_model_Enhanced_4(Time_Limit):
